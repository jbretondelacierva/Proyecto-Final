...
let cogerDatos = async () => {
    ...
    let request = await fetch("/api/v1/eventos/"+nombre);
...
const postEventos = async () => {
    let user_name= await getUsuario();

    try {
        let inputPeriodo = document.getElementById("Nombre").value;
        console.log(inputPeriodo);
        let inputConcepto = document.getElementById("Descripcion").value;
        console.log(inputConcepto);
        let inputImporte = document.getElementById("Mes").value;
        ...
        var data = {
            ...
            "username":user_name,
            "nombre":inputPeriodo,
            "descripcion":inputConcepto,
            "mes":inputImporte,
            };

            const address = 'api/v1/eventos';
